    ...
    const fetchData = async () => {
      try {
        ...
        if (!chatSnapshot.empty) {
          const chatDoc = chatSnapshot.docs[0];
          const chatDocRef = doc(chatRef, chatDoc.id);
          const val = await getDoc(chatDocRef);
          const data = val.data();
          setMessages(data);
        }
      } catch (error) {
  ...
  return (
    <>
      <div className=" login-form flex flex-row text-white h-screen fixed w-full bg-gray-700 ">
        ...
        <div class="section-2 login-form flex-1 overflow-y-auto w-6/12 ">
                  ...
                  <section key={index} className="Messages ">
                    {data.senderId === userId ? (
                      ...
                      <div className="flex justify-start space-x-1 m-1">
                        ...
                        />
                        <div className="bg-slate-600 rounded-lg p-1">{data.message}</div>
                      </div>
                    )}
                  </section>
                ))}
          ...
          <div className="flex justify-center content-center">
            <div className="NavbarBottom fixed bottom-0 max-w-md md:w-full md:max-w-screen-sm p-2 ">
              <form className="flex w-full">
                ...
                />
                <button
                  className="rounded-full m-1 bg-gray-800 p-2"
                  type="submit"
                  onClick={(e) => {
                    e.preventDefault();
                    sendMessage();
